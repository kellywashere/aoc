...
bool* run_robot(struct computer* comp) {
	// create grid
	bool* color = calloc(GRIDW * GRIDH, sizeof(bool));
	int dir = 0;
	int x = 0;
	int y = 0;
	int xmin = x;
	...
	while (comp->mem[comp->pc] != HLT) {
		...
		if (x < 0 || x >= GRIDW || y < 0 || y >= GRIDH) {
			fprintf(stderr, "ERROR: running off grid: x,y = %d,%d; min-max: %d,%d - %d,%d\n", x, y,
					xmin, ymin, xmax, ymax);
			return color;
		}
	}
...
int main(int argc, char* argv[]) {
	...
	if (do_disassembly)
		...
		bool* color = run_robot(&comp);
		for (int y = 0; y < GRIDH; ++y) {
			for (int x = 0; x < GRIDW; ++x) {
				printf("%c", color[y * GRIDW + x] ? '#' : ' ');
			}
			printf("\n");
		}
		free(color);
