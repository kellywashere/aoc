...
void run_program(struct computer* comp, int inp, word* out1, word* out2) {
	int outputs_given = 0;
	bool input_given = false; // for bug detection purposes
	while (computer_get_mem(comp, comp->pc) != HLT) {
		...
		switch (opcode) {
			...
			case INP:
				if (input_given)
					printf("WARNING: Input was already given!\n");
				addr = computer_get_mem(comp, comp->pc + 1) + (mode[0] == 2 ? comp->base : 0);
				computer_set_mem(comp, addr, inp);
				comp->pc += 2;
				input_given = true;
				break;
			case OUT:
...
int run_robot(struct computer* comp) {
	// create grid
	bool* color = calloc(GRIDWH * GRIDWH, sizeof(bool));
	bool* painted = calloc(GRIDWH * GRIDWH, sizeof(bool));
	int dir = 0;
	int x = GRIDWH / 2;
	int y = GRIDWH / 2;
	word out1, out2;
	while (comp->mem[comp->pc] != HLT) {
		int idx = y * GRIDWH + x;
		int in = color[idx] ? 1 : 0;
		run_program(comp, in, &out1, &out2);
		// printf("%d: %" PRId64 ", %" PRId64 "\n", in, out1, out2);
		color[idx] = out1 == 1;
		painted[idx] = true;
		dir = (dir + (out2 == 0 ? 1 : 3)) % 4;
		x += dir2dx[dir];
		y += dir2dy[dir];
		if (x < 0 || x >= GRIDWH || y < 0 || y >= GRIDWH) {
			fprintf(stderr, "ERROR: running off grid: x,y = %d,%d\n", x, y);
			return -1;
		}
	}
	// count painted tiles
	int count = 0;
	for (int ii = 0; ii < GRIDWH * GRIDWH; ++ii)
		count += painted[ii] ? 1 : 0;
	free(color);
	free(painted);
	return count;
}
