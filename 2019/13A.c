...
void run_program(struct computer* comp, word out[3]) {
	int outputs_given = 0;
...
int main(int argc, char* argv[]) {
	...
	if (do_disassembly)
		...
		enum cell* grid = create_and_fill_grid(&comp);
		// show_grid(grid);
		int count = 0;
		for (int ii = 0; ii < GRIDW * GRIDH; ++ii)
			count += grid[ii] == BLOCK ? 1 : 0;
		printf("%d\n", count);
		free(grid);
	}
