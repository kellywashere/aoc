...
int count_occupied(struct grid* g, int x, int y) {
	// count occupied seats in 3x3 grid centered at x,y
	int count = 0;
	for (int dy = -1; dy <= 1; ++dy) {
		...
		if (yy >= 0 && yy < g->gridh) {
			for (int dx = -1; dx <= 1; ++dx) {
				int xx = x + dx;
				bool occ = xx >= 0 && xx < g->gridw && g->grid[yy * g->gridw + xx] == OCCUPIED;
				count += occ ? 1 : 0;
			}
		}
	}
	...
	for (int y = 0; y < in->gridh; ++y) {
		for (int x = 0; x < in->gridw; ++x) {
			...
			if (in->grid[idx] != FLOOR) {
				...
				else if (in->grid[idx] == OCCUPIED && occ >= 5) { // 5 inc this chair
					out->grid[idx] = EMPTY;
					changed = true;
				}
			}
