...
// check if we can construct correct sum from remaining packages to create group 2 (and 3)
bool check_group2(struct package* p, int nr_packages, int idx0, int sum_left) {
	if (sum_left == 0) return true;
	if (sum_left < 0) return false;
	if (idx0 >= nr_packages) return false;
	if (p[idx0].weight > sum_left) return false; // nrs are sorted
	if (check_group2(p, nr_packages, idx0 + 1, sum_left)) // do not include p[idx0] in sum
		return true;
	return check_group2(p, nr_packages, idx0 + 1, sum_left - p[idx0].weight); // include p[idx0]
}
...
void find_group1(struct package* p, int nr_packages, int idx0, int sum, int target_sum, int used, struct group1score* best_score) {
	if (sum == target_sum && used <= best_score->nrpackages) { // found an option
		if (check_group2(p, nr_packages, 0, target_sum)) {
...
int main(int argc, char* argv[]) {
	...
	for (int ii = 0; ii < nr_packages; ++ii)
		sum += p[ii].weight;
	if (sum % 3 != 0) {
		fprintf(stderr, "Sum not divisible by 3!\n");
		return 1;
	}
	sum /= 3;
