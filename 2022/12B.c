...
// 12B: run reverse search starting from end-point, until we find an 'a'
		...
		if (g->grid[idx].height == 0)
			...
			for (int dir = 0; dir < 4; ++dir) { // 4 neighboring cells
				...
				if (nx >= 0 && nx < w && ny >= 0 && ny < h) {
					...
					if (g->grid[nidx].height >= g->grid[idx].height - 1) { // Note: reverse search is done here!!
