...
#define ROUTE_TOP_SZ 13 /* take top ? of vertices to permutate route */
...
int find_max_reward(struct graph* g, int posidx, int turns_left) {
	// recurse solution algorithm
	struct vertex* vcur = &g->vertex[posidx];
	...
	for (int e_idx = 0; e_idx < vcur->nr_edges; ++e_idx) {
		...
		if (vto->flow && !vto->open && e->len + 1 < turns_left) { // otherwise no point in going there
			...
			vto->open = true;
			int turns_left_next = turns_left - e->len - 1;
			int reward = turns_left_next * vto->flow;
			reward += find_max_reward(g, e->to, turns_left_next);
...
int main(int argc, char* argv[]) {
	...
	int curidx = 0; // start node by design
	int turns_left = 30;
	int max_reward = find_max_reward(g, curidx, turns_left);
	printf("%d\n", max_reward);
