...
struct grid* grid_copy(struct grid* g) {
	...
	memcpy(newg, g, sizeof(struct grid));
	newg->rowcapacity = g->gridh;
	newg->grid = malloc(newg->gridw * newg->rowcapacity * sizeof(struct cell));
	memcpy(newg->grid, g->grid, g->gridw * g->gridh * sizeof(struct cell));
	return newg;
}
...
void reset_entities(struct grid* g, struct entity* entities[], int nr_entities) {
	// call with original grid before running game loop
	for (int y = 0; y < g->gridh; ++y) {
		for (int x = 0; x < g->gridh; ++x) {
			int idx = y * g->gridw + x;
			if (g->grid[idx].type == ENTITY) {
				struct entity* e = g->grid[idx].entity;
				e->x = x;
...
int main(int argc, char* argv[]) {
	...
	int elf_attack = 4;
	while (!elves_won) {
		struct grid* grid_cpy = grid_copy(grid);
		grid_cpy->rounds_played = 0;
		reset_entities(grid, entities, nr_entities);
		// set elf attack
		for (int ii = 0; ii < nr_entities; ++ii)
			if (entities[ii]->type == ELF)
				entities[ii]->attack = elf_attack;

		run_game(grid_cpy, entities, nr_entities);
		// check if all elves are still alive
		elves_won = true;
		for (int ii = 0; elves_won && ii < nr_entities; ++ii)
			if (entities[ii]->type == ELF)
				elves_won = entities[ii]->hp > 0;

		if (elves_won) {
			printf("Elves won when attack was boosted to %d\n", elf_attack);
			int hp_remaining = 0;
			for (int ii = 0; ii < grid_cpy->gridw * grid_cpy->gridh; ++ii)
				hp_remaining += grid_cpy->grid[ii].type == ENTITY ? grid_cpy->grid[ii].entity->hp : 0;
			int prod = grid_cpy->rounds_played * hp_remaining;
