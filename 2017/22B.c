...
void grid_add_line(struct grid* g, char* line) {
	...
	if (!g->grid) {
		...
		g->grid = calloc(g->gridw * g->rowcapacity, sizeof(char));
	}
	if (g->gridh >= g->rowcapacity) {
		g->grid = realloc(g->grid, g->gridw * 2 * g->rowcapacity * sizeof(char));
		for (int ii = g->gridw * g->rowcapacity; ii < g->gridw * 2 * g->rowcapacity; ++ii)
			g->grid[ii] = false;
		g->rowcapacity *= 2;
	}
	for (int col = 0; col < g->gridw; ++col)
		g->grid[col + g->gridh * g->gridw] = line[col] == '#' ? 2 : 0;
	++g->gridh;
	g->posx = (g->gridw - 1) / 2;
	g->posy = (g->gridh - 1) / 2;
}
...
struct grid* enlarge_grid(struct grid* g, int newsize) {
	...
	ng->grid = calloc(newsize * newsize, sizeof(char));
...
void step(struct grid** pG) {
	struct grid* g = *pG;
	char v = g->grid[g->posy * g->gridw + g->posx];
	g->grid[g->posy * g->gridw + g->posx] = (v + 1) % 4;
	g->infection_count += v == 1 ? 1 : 0;
	if (v == 0 || v == 2) {
		int rot = v == 0 ? -1 : 1; // -1: left, 1: right
...
int main(int argc, char* argv[]) {
	// grid = calloc(GRIDSIZE * GRIDSIZE, sizeof(char));
