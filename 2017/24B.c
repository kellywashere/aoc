...
void find_largest_bridge(struct comp* comps, int nr_comps, int p, int* sum, int* length) {
	int largest_len = 0;
	int largest_sum = 0;
	for (int ii = 0; ii < nr_comps; ++ii) {
		if (comps[ii].used)
		...
		for (int portnr = 0; portnr <= 1; ++portnr) {
			if (comps[ii].p[portnr] == p) {
				comps[ii].used = true;
				int l, s;
				find_largest_bridge(comps, nr_comps, comps[ii].p[1 - portnr], &s, &l);
				l += comps[ii].length;
				s += comps[ii].p[0] + comps[ii].mid + comps[ii].p[1];
				if (l > largest_len || (l == largest_len && s > largest_sum)) {
					largest_len = l;
					largest_sum = s;
				}
				comps[ii].used = false;
			...
			if (p != 0) {
				...
				if (count == 2) { // find the idx of the other one
					...
					if (idx < *nr_comps) {
						...
						comps[ii].length += comps[idx].length;
						--(*nr_comps);
						for (; idx < *nr_comps; ++idx)
...
int main(int argc, char* argv[]) {
	...
	int maxlen, maxsum;
	find_largest_bridge(comps, nr_comps, 0, &maxsum, &maxlen);
	printf("%d\n", maxsum);
